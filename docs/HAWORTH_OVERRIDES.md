...
| H-024 | Removed | Chain2 fan-out solver deleted; replaced with standard 3-step connector path matching the HO arm and all simple OH connectors (`direction_policy="line"`). | --- | --- | --- | --- |
| H-025 | Removed | Standoff floor deleted; `branch_standoff` uses `segment_length` directly without `min_standoff_factor` override. | --- | --- | --- | --- |
