...
#    dom_ext.safe_indent( root)
		...
		import shutil
		import tempfile
		temp_dir = tempfile.mkdtemp(prefix="bkchem_odf_")
		try:
			cfname = os.path.join( temp_dir, "content.xml")
			with open(cfname, 'wb') as f:
				f.write(out.toxml('utf-8'))
			sfname = os.path.join( temp_dir, "styles.xml")
	...
	def create_oo_text( self, ftext, coords, page, para_style_name, txt_style_name, gr_style_name):
		...
		to_parse = safe_xml.parse_dom_from_string( ftext).childNodes[0]
...
class text_style( style):
	...
	def to_dom( self, doc):
		...
		dom_extensions.setAttributes( style, (('style:family', self.family),
																					('style:name', self.name)))
		dom_extensions.elementUnder( style, 'style:text-properties', (( 'fo:font-size', self.font_size),
																																	( 'fo:font-family', self.font_family),
