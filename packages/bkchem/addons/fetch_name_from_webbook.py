...
def stereoisomers_found(app, streamx):
	alt_mol = []
	for line in streamx:
	...
	w = Pmw.ScrolledListBox(
		dialog.interior(),
		labelpos='n',
		items=mols,
		label_text='These Stereoisomers were found',
	)
	w.pack(expand=1, fill='both', padx=4, pady=4)
	dialog.activate()
	for a in alt_mol:
		if a[0] == w.getvalue()[0]:
			return download_nist(app, "http://webbook.nist.gov" + a[1])


#Searches nist by name using inchi instead of the name
def get_name_from_webbook(app, inchi):
	...
	url = "http://webbook.nist.gov/cgi/cbook.cgi?Name=%s&Units=SI" % (
		"+".join(searchstr.split()),
	)
	return download_nist(app, url)


def download_nist(app, url):
	#print url
	...
	for line in stream_lines:
		...
		if line == "<h2>No matches found</h2>\n":   #When stereoisomers are found
			dialog.close()
			return stereoisomers_found(app, stream_lines)
...
def err_mess_box(app, mess):
	#Pops up error OK-box
	message = ""
	for m in mess:
		message = message + m + "\n"
	dialog = Pmw.Dialog(app.paper, buttons=('OK',), defaultbutton='OK', title='Error')
...
def main(app):
	...
	if len(a) != 0:
		for b in a:
			...
			if inchi_form:
				mess = []
				mol_name, cas, mess = get_name_from_webbook(app, inchi_form)
				err_mess = err_mess + mess
		...
		if len(err_mess) != 0:   #check for any error messages
			err_mess_box(app, err_mess)
		app.paper.add_bindings()
	else:
		err_mess_box(app, ["Please select a molecule"])
