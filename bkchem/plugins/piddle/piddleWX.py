# piddleWX.py -- a wxPython backend for PIDDLE
...
class _WXCanvasDefaultStatusBar(wxStatusBar):
  ...
  def Reposition(self):
    ...
    field = self.GetFieldRect(1)
    #self.click.SetPosition(wxPoint(field.x + field.width/2 -20, field.y)) # original
    self.click.SetPosition(wxPoint(field.x+15, field.y))
    #self.click.SetSize(wxSize(100, field.height)) # original but try using defaults instead
